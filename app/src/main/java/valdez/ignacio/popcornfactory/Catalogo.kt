...
class Catalogo : AppCompatActivity() {
    ...
    class ItemAdapter:BaseAdapter{
        ...
        override fun getView(p0: Int, p1: View?, p2: ViewGroup?): View {
            ...
            var vista = inflator.inflate(R.layout.item, null)

            var image = vista.findViewById(R.id.cell_itemImage) as ImageView
            var title = vista.findViewById(R.id.cell_itemTitle) as TextView

            image.setImageResource(item.image)
            title.setText(item.titulo)

            image.setOnClickListener {
                var intent = Intent(context, detalle_item::class.java)
                intent.putExtra("nombre", item.titulo)
